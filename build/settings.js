...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetchButton.addEventListener("click", function fetchButtonHandler(e) {
    ...
    fetch(`${ajaxurl}?action=gutailberg_get_fse_templates`).then(res => res.json()).then(({
      ...
      document.getElementById("fse-templates").innerHTML = data.contents.replaceAll(regex, subst);
  ...
  generateButton.addEventListener("click", function (e) {
    ...
    if (!matchingElement) {
      console.log("There is something wrong extracting Tailwind CSS from the current page.");
    }
